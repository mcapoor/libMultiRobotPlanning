...
class ShortestPathHeuristic {
 public:
  // The search graph is only written to `dotFile` (graphviz) if it is given, for
  // debugging: every solver run would otherwise write it to the working
  // directory, and parallel runs would overwrite each other's file.
  ShortestPathHeuristic(size_t dimx, size_t dimy,
    ...
    if (!dotFile.empty()) {
      writeDotFile(searchGraph, dotFile);
    }

...
# colors of the paths: neither the orange of the agents nor the red of the obstacles and collisions
PathColors = ['tab:blue', 'tab:green', 'tab:purple', 'tab:brown', 'tab:pink', 'tab:gray', 'tab:olive', 'tab:cyan']
...
class Animation:
  def __init__(self, map, schedule):
    ...
    # then the path of each agent, under the agents
    for d, i in zip(map["agents"], range(0, len(map["agents"]))):
      waypoints = [(w["x"], w["y"]) for w in schedule["schedule"][d["name"]]]
      self.patches.append(PathPatch(Path(waypoints), facecolor='none', edgecolor=PathColors[i%len(PathColors)], linewidth=3, alpha=0.7, capstyle='round', joinstyle='round'))
...
if __name__ == "__main__":
  ...
  with open(args.map) as map_file:
    map = yaml.load(map_file, Loader=YAML_LOADER)

  with open(args.schedule) as states_file:
    schedule = yaml.load(states_file, Loader=YAML_LOADER)
